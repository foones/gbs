...
class GbsLexer(lang.bnf_parser.Lexer):
    ...
    def tokenize(self, string, filename='...'):
        ...
        for tok in supertok:
            ...
            elif len(self.token_mem) >= 3 and self.token_mem[-3].type == 'BOOM' and self.token_mem[-1].type != 'string':
                raise GbsParserException(i18n.i18n('BOOM can only accept a string'), area)
...
class GbsParser(lang.bnf_parser.Parser):
    ...
    def parse_error(self, nonterminal, previous_token, token):
        ...
        if previous_token.type == 'lowerid' and token.type == '(':
            ...
            l2 = i18n.i18n('Strings can only be used in a BOOM command')
            raise GbsParserException('\n'.join([l1, l2]), area)
        elif token.type == 'EOF':
...
Analyzer = GbsAnalyzer(read_file(lang.GbsGrammarFile))
