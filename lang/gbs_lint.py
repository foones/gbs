...
subprograms, given an abstract syntax tree."""
    ...
    def _check_Main(self, tree):
        last_is_main = self._is_Main_def(tree.children[-1])

        there_is_main = False
        main_tree = None
        for def_ in tree.children:
            if self._is_Main_def(def_):
                there_is_main = True
                main_tree = def_

        if there_is_main and not last_is_main:
            pos = common.position.ProgramAreaNear(main_tree)
            raise GbsLintException(i18n.i18n('The Main procedure should be the last definition'), pos)

        if self.is_main_program and not there_is_main:
            pos = common.position.ProgramAreaNear(tree.children[-1])
            raise GbsLintException(i18n.i18n('There should be a Main procedure'), pos)

    def _is_Main_def(self, def_):
        return len(def_.children) > 2 and \
                      def_.children[0] == 'procedure' and \
                      self._is_upperid('Main', def_.children[1])
    ...
    def _add_var(self, varName, tree):
        ...
        if var is None:
            ...
            msg = i18n.i18n('Cannot modify "%s": index of a repeatWith is immutable') % (varName,)
            area = common.position.ProgramAreaNear(tree)
            raise GbsLintException(msg, area)
    ...
    def _add_index(self, varName, tree):
        ...
        if var is None:
            ...
            msg = i18n.i18n('Index of a repeatWith cannot be a variable: "%s"') % (varName,)
            area = common.position.ProgramAreaNear(tree)
            raise GbsLintException(msg, area)
        elif var.type() == 'parameter':
            msg = i18n.i18n('Index of a repeatWith cannot be a parameter: "%s"') % (varName,)
            area = common.position.ProgramAreaNear(tree)
            raise GbsLintException(msg, area)
    ...
    def _check_nested_indexes(self, varName, tree):
        if self.symbol_table.is_immutable(varName):
            msg = i18n.i18n('Nested repeatWith\'s cannot have the same index: "%s"') % (varName,)
            area = common.position.ProgramAreaNear(tree)
            raise GbsLintException(msg, area)
    ...
    def check_definition1(self, tree):
        ...
        if prfn == 'procedure' and name == 'Main':
            self.check_main_definition(tree)
        elif prfn == 'procedure':
    ...
    def check_main_definition(self, tree):
        prfn, name, params, body, typeDecl = tree.children
        if len(params.children) > 0:
            area = common.position.ProgramAreaNear(params)
            msg = i18n.i18n('Main procedure cannot have parameters')
            raise GbsLintException(msg, area)
        if self._body_has_return(body):
            return_clause = body.children[-1]
            if not self._returns_varTuple(return_clause):
                area = common.position.ProgramAreaNear(return_clause)
                msg = i18n.i18n('Main procedure should only return variables')
                raise GbsLintException(msg, area)
