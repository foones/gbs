  ...
  def compile_routine(self, prog, rtn):
    ...
    while _op_i < len(rtn.ops):
      ...
      if opcode == 'pushConst':
        ...
        self._program.add(self._arch.BOOM(common.utils.show_string(op[1])))
...
class JitCompiledRunnable(lang.gbs_runnable.GbsRunnable):
  ...
  def run(self, board):
    ...
    resbuf_len = max(arch.Word_size * self._nretvals, arch.BOOM_Max_err_len)
