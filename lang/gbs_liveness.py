...
modifies the current state of the analysis."""
  ...
  def annotate_cmd(self, tree, next_block_in):
    ...
    dispatch = {
      ...
      'repeatWith': self.annotate_repeatWith,
  ...
  def annotate_repeatWith(self, tree):
    var = tree.children[1].value
    kill = tokset_new([var])
    self._live_out_loop(tree, minus=kill) # add in to out
    # in(repeatWith i in from..to body)
    #   = gen(from) U gen(to) U (in(body) \ {i}) U out(repeatWith ...)
...
that each defined variable (except parameters and repeatWith
indexes) is used later."""
  ...
  def check_unitialized(self, tree):
    params = [p.value for p in tree.children[2].children]
    block = tree.children[3]
  ...
  def check_unused_def(self, tree):
    self.check_unused_commands(tree.children[3])
  ...
  def check_unused_cmd(self, tree):
    ...
    dispatch = {
      ...
      'repeatWith': self.check_unused_repeatWith,
