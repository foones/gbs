  ...
  def step(self):
    ...
    if opcode == 'pushConst':
      ...
      if len(self.callstack) == 0:
        assert self.ar.routine.name == 'Main'
...
def interp(compiled_program, board):
  vm = GbsVmInterpreter()
  vm.init_program(compiled_program, board)
  while True:
    r = vm.step()
    if r[0] == 'END':
      return r[1]
...
class VmCompiledRunnable(lang.gbs_runnable.GbsRunnable):
  ...
  def run(self, board):
    return interp(self._prog, board)
